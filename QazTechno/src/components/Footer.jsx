...
function Footer() {
  ...
  const linkSections = [
    { title: t("footer.marketplace"), items: aboutLinks },
    { title: t("footer.earn_with_us"), items: earnWithOzonLinks },
    { title: t("footer.help"), items: helpLinks },
  ];


  console.log(t("footer.about_ozon"));
    ...
    <footer className="w-full">
      ...
      <div className="hidden md:grid container-custom mt-6 rounded-md p-4 bg-[#e0e8f0] grid-cols-[1fr_3fr]">
        ...
        <div className="flex flex-wrap gap-6 items-center">
         {categories.map(({ icon: Icon, text }, index) => (
            <div key={index} className="flex gap-2 items-center">
              <Icon className="text-[#99a3ae]" />
              <p className="font-pop text-[1rem] font-medium text-[#101820]">{text}</p>
            </div>
         ))}
        </div>
      </div>
        ...
        <div className="py-6 px-10 border rounded-2xl text-center max-w-[300px] mx-auto md:mx-0">
          ...
          <div className="flex justify-center gap-6">
            {[Apple, PlayMarket, Huawai].map((img, i) => (
              <img key={i} src={img} className="w-10" alt={t("footer.store_alt")} />
            ))}
          </div>
        </div>
      ...
      <div className="container-custom mt-10 flex flex-col sm:flex-row justify-between items-center gap-4 p-4 border-t border-gray-300">
        ...
        <div className="flex items-center gap-4">
          ...
          <button className="btn-primary ml-4">{t("footer.have_questions")}</button>
        </div>
      </div>
    </footer>
  );
